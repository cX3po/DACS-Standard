...
For the siwd kind, the SIWD message string MUST include a "Resource" line per EIP-4361 whose value is the RFC 3986 URI `dacs:<hex>`, where `<hex>` is the lowercase hex encoding of the canonical signed_bytes value (i.e. `dacs:${hexEncode(signedBytes("bundle-presentation", bundle_hash))}`, with the `dacs-bundle-presentation:v1:` domain-separation prefix carried INSIDE the hashed-and-hex-encoded signed_bytes, NOT in the URI scheme path). This is the exact resource string the reference `docs/flow-trace.md` emits and re-derives for its `bundle.presentation.message.includes(expectedResource)` SIWD check, so the normative text and the reference implementation produce byte-identical resources. The bare hex value MUST NOT be emitted on its own (it is not a valid RFC 3986 URI and a strict EIP-4361 parser rejects the whole SIWD message); only the `dacs:<hex>` form is conformant. The SIWD signature thereby transitively binds to the same payload (the message is the SIWD envelope, the resource carries the bundle binding). For the per-claim kind, each per-claim signature signs signed_bytes (not the raw bundle hash). For the session-key kind, the session key signs signed_bytes; if rootBinding is set, the root key’s binding signature signs "dacs-session-binding:v1:" || session_key || bundle_hash. For the sr1-root kind, the SR-1 aggregate signature signs signed_bytes; verifiers reconstruct the SR-1 aggregate from the rootClaim’s sub-identity set and verify against signed_bytes.
